...
- **CUDA is experimental and explicit.** Enable `tenferro-cuda` on `tensor4all-core` and `tensor4all-treetn`, then follow [`docs/book/src/tutorials/gpu/cuda-tree-contraction.md`](../../docs/book/src/tutorials/gpu/cuda-tree-contraction.md). The current surface is ordinal-0 upload, dense untracked same-dtype TreeTN full contraction, and download—never assume automatic transfer, CPU fallback, SVD/QR/truncation, TCI/ACI, or multi-GPU support.
- **`tensor4all-tensorbackend` is internal** — never depend on it or instantiate `CpuBackend` directly; get scalars, storage, and linalg through the public crates. `tensor4all-core` hosts the TCI substrate (`CachedFunction`, `MultiIndex`, rrLU/LUCI) since the former `tensor4all-tcicore` was dissolved into it: depend on core for `CachedFunction`, or take `MultiIndex` re-exported from `tensor4all-partitionedtt`.
...
- **`target/api-dump/*.md` in a repository checkout** — generated and completeness-checked by `cargo run -p xtask --release -- api-dump`. Read the relevant generated file when present; never hand-edit or commit it.
...
**SVD truncation uses `rtol`, not `cutoff`.** ITensors.jl uses `cutoff`; convert with `rtol = sqrt(cutoff)` (so `cutoff=1e-10` → `rtol=1e-5`). Algorithm-specific option structs such as `CompressionOptions` and `TCI2Options` instead name their threshold `tolerance`; check the exact type. **Exception: `tensor4all-partitionedtreetn`'s adaptive surface uses `cutoff` directly** (ITensorMPS convention): `PatchingOptions::cutoff` and `truncate_adaptive(..., cutoff, ...)` are a local discarded-weight cutoff, 1:1 with ITensors `cutoff` (old root-relative value translates as `cutoff = old_rtol²`), best effort for the final whole-network error — never a bound. The deprecated `tensor4all-partitionedtt` keeps `rtol`. Singular values are indexed `s[[0, i]]`, not `s[[i, i]]`.
...
- **`TensorTrain` / `MPS` / `MPO` share one representation.** In `tensor4all-itensorlike`, `TensorTrain` (tree-based), `MPS`, and `MPO` are the same underlying type; MPS-like (1 site index per node) vs MPO-like (2 site indices per node) is a runtime property (`is_mps_like`/`is_mpo_like`), not a type. `tensor4all-simplett`'s positional chain type is a *different* type named `SimpleTensorTrain`. Use the representation-specific error type: `tensor4all_simplett::SimpleTensorTrainError` for `SimpleTensorTrain`, or `tensor4all_itensorlike::TensorTrainError` for the tree-based `TensorTrain`.
...
- **Bond cap / tolerance vocabulary.** The bond-dimension cap is `max_bond_dim: Option<usize>` (`None` = unlimited) everywhere — ITensors.jl `maxdim` maps to `max_bond_dim: Some(d)`, and `cutoff` maps to `rtol` with `rtol = sqrt(cutoff)`. See `docs/book/src/conventions.md`. Exception: `tensor4all-partitionedtreetn` adaptive patching uses `cutoff` directly (local discarded weight, best effort, no global bound).
...
- Generated API dump: `target/api-dump/` (from `cargo xtask api-dump`; do not edit or commit).
